// Word list sourced from https://gist.github.com/shmookey/b28e342e1b1756c4700f42f17102c2ff
...
// ---------------------------------------------------------------------------
// Game state (mutated by game functions)
// ---------------------------------------------------------------------------
let targetWord = '';

// Letters the player is still allowed to use (available + required)
let availableLetters = new Set();

// Subset of available letters: these MUST appear in every subsequent guess
// because they appeared in the target word in a previous guess
let requiredLetters = new Set();

// Log of all guesses: [{ word: string, result: string[] }]
let guesses = [];
...
// ---------------------------------------------------------------------------
/**
 * Compares `guess` against `target` using standard Wordle two-pass logic.
 * Returns an array of 5 strings: 'green' | 'yellow' | 'gray'.
 */
function evaluateGuess(guess, target) {
  const result = new Array(WORD_LENGTH).fill('gray');
  const targetRemaining = target.split('');
  const guessArr = guess.split('');

  // Pass 1 – exact matches (green)
  for (let i = 0; i < WORD_LENGTH; i++) {
    if (guessArr[i] === targetRemaining[i]) {
      result[i] = 'green';
      targetRemaining[i] = null; // mark consumed
    }
  }
  ...
  for (let i = 0; i < WORD_LENGTH; i++) {
    ...
    if (idx !== -1) {
      result[i] = 'yellow';
      targetRemaining[idx] = null; // mark consumed
    }
  }
...
function submitGuess() {
  ...
  if (guess.length !== WORD_LENGTH) {
    showMessage(`Please enter a ${WORD_LENGTH}-letter word.`, 'error');
    return;
  }
  ...
  // ---- letter-pool validation ----
  const uniqueLetters = new Set(guess.split(''));

  for (const letter of uniqueLetters) {
    if (!availableLetters.has(letter)) {
      showMessage(`"${letter}" has already been used — choose another letter.`, 'error');
      return;
    }
  }

  // ---- required-letters validation ----
  for (const letter of requiredLetters) {
    if (!guess.includes(letter)) {
      showMessage(
        `"${letter}" was found in a previous guess and must appear in every subsequent guess.`,
        'error'
      );
      return;
    }
  ...
  // ---- update letter pools ----
  //   1. Remove every letter used in this guess from available
  //   2. Letters that hit (green/yellow) become required going forward
  //   3. Required letters are kept in available so they can be reused
  for (const letter of uniqueLetters) {
  ...
  // Re-add required letters so they remain usable
  for (const letter of requiredLetters) {
  ...
  if (result.every((r) => r === 'green')) {
    // Player accidentally (or intentionally) solved the word
    endGame(true);
  } else if (!canMakeValidGuess()) {
    // No valid word can be formed with remaining letters
    endGame(false);
  }
}
...
function endGame(solved) {
  ...
  if (solved) {
    showMessage(
      `You solved it in ${guesses.length} guess${guesses.length === 1 ? '' : 'es'} — the word was ${targetWord}. Try to do worse next time!`,
      'win'
    );
  } else {
    showMessage(
      `Game over! You made ${guesses.length} guess${guesses.length === 1 ? '' : 'es'}. The word was ${targetWord}.`,
      'gameover'
    );
  }
}
...
/**
 * Returns true if at least one word in the list can be composed entirely
 * from `availableLetters` AND contains every letter in `requiredLetters`.
 */
function canMakeValidGuess() {
  return WORDS.some((word) => {
    // Every letter in this word must still be available
    for (const letter of word) {
      if (!availableLetters.has(letter)) return false;
    }
    // Every required letter must appear in the word
    for (const letter of requiredLetters) {
...
function renderBoard() {
  ...
  // Auto-scroll board to the latest guess
  ...
  for (const rowLetters of KEYBOARD_ROWS) {
    ...
    for (const letter of rowLetters) {
      ...
      if (requiredLetters.has(letter)) {
        ...
        key.title = `${letter} — required in every future guess`;
      } else if (!availableLetters.has(letter)) {
        key.classList.add('used');
        key.title = `${letter} — already used`;
      }
...
function updateInfoPanel() {
  const reqEl = document.getElementById('required-letters');
  const avlEl = document.getElementById('available-letters');

  const req = [...requiredLetters].sort();
  // Show available letters that are NOT in the required set separately
  const avl = [...availableLetters].filter((l) => !requiredLetters.has(l)).sort();

  reqEl.textContent = req.length ? req.join('  ') : '—';
  avlEl.textContent = avl.length ? avl.join('  ') : '—';
}
